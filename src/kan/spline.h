...
Returns:
    Tensor: B-spline bases tensor of shape (in_features, grid_size + spline_order).
*/
...
Tensor b_splines(Tensor& grid, Tensor& Xs, uint32_t spline_order, Tensor& coeff) {
	...
	float *bases_minus_1_data = new float[in_features * (num_grid_points - spline_order)];
	float *bases_data = new float[in_features * (num_grid_points - spline_order - 1)];
	...
	Tensor bases_minus_1({in_features, num_grid_points - spline_order}, bases_minus_1_data);
