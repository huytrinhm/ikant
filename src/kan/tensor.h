...
std::ostream& operator<<(std::ostream &os, const Tensor &tensor) {
    tensor.print_tensor_data(os, tensor.dim, tensor.shape, tensor.stride, tensor.data);
    return os;
}
...
Tensor tensor_from_file(const char *filename) {
    ...
    fin.read(reinterpret_cast<char*>(result.shape), result.dim * sizeof(uint32_t));
    fin.read(reinterpret_cast<char*>(result.stride), result.dim * sizeof(uint32_t));
...
void tensor_to_file(Tensor& tensor, const char *filename) {
    ...
    fout.write(reinterpret_cast<char*>(tensor.shape), tensor.dim * sizeof(uint32_t));
    fout.write(reinterpret_cast<char*>(tensor.stride), tensor.dim * sizeof(uint32_t));
    ...
    fout.write(reinterpret_cast<char*>(tensor.data), num_elements * sizeof(float));
