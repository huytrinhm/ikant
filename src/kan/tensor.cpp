...
Tensor::Tensor(uint32_t dim, uint32_t* shape, uint32_t* stride, float* data)
...
Tensor::Tensor(std::initializer_list<uint32_t> shape_init, float* data)
    : data(data) {
  dim = shape_init.size();
...
Tensor::Tensor(std::initializer_list<uint32_t> shape_init) {
  dim = shape_init.size();
...
void Tensor::print_tensor_data(std::ostream& os,
                               uint32_t _dim,
                               uint32_t* shape,
                               uint32_t* stride,
                               float* data) const {
...
GC_Tensor::GC_Tensor(Tensor&& other) : Tensor(other) {}

GC_Tensor& GC_Tensor::operator=(Tensor&& other) {
  delete[] shape;
  delete[] stride;
  delete[] data;
